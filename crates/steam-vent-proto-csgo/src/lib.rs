//! Protobufs for steam-vent for Counter-Strike 2 / Counter-Strike: Global
//! Offensive (app id 730) game-coordinator messages.
//!
//! Like [`steam_vent_proto_steam`], the protobuf surface is generated and
//! consumed with [`prost`]: every message is a plain prost struct and the glue
//! ([`RpcMessageWithKind`]/[`MsgKindEnum`]) is emitted by
//! [`steam-vent-proto-build`]. The `.proto` sources have no `package`, so prost
...
//! `steam-vent` (`Connection::game_coordinator`); [`GCHandshake`] is the typed
//! handshake for app 730.
//!
//! Two upstream `.proto` files are intentionally **not** vendored here:
//! `enums_clientserver.proto` (the full Steam-client `EMsg`) and
//! `steammessages_base.proto`. Neither is imported by any csgo game-coordinator
...
//! provided by [`steam_vent_proto_steam`]. See `docs/RESEARCH.md` §8.
//!
//! [`steam_vent_proto_steam`]: https://docs.rs/steam-vent-proto-steam
//! [`prost`]: https://docs.rs/prost
//! [`steam-vent-proto-build`]: https://docs.rs/steam-vent-proto-build
