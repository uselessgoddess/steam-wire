...
impl Header {
    pub fn validate(&self) -> Result<()> {
        if self.magic != MAGIC { Err(NetworkError::InvalidHeader) } else { Ok(()) }
    }
}
...
impl Encoder<RawNetMessage> for RawMessageEncoder {
    ...
    fn encode(&mut self, mut item: RawNetMessage, dst: &mut BytesMut) -> Result<(), Self::Error> {
        ...
        let iv_buffer = item.iv_buffer.unwrap_or_else(|| BytesMut::from(&[0; 16][..]));
        ...
        let mut buf = item.frame_header_buffer.unwrap_or_else(|| BytesMut::from(&[0; 8][..]));
...
pub async fn connect<A: ToSocketAddrs + Debug>(
    ...
    let response =
        ClientEncryptResponse { protocol: encrypt_request.protocol, encrypted_key: key.encrypted };
