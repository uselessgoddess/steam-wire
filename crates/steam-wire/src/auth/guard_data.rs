...
pub enum FileStoreError {
    ...
    Json { err: serde_json::error::Error, path: PathBuf },
...
impl FileGuardDataStore {
    ...
    ///
    /// This will be
    /// - `$XDG_CACHE_HOME/steam-wire/machine_token.json` (where `$XDG_CACHE_HOME` defaults to `$HOME/.cache`) on Linux
    /// - `$HOME/Library/Caches/steam-wire/steam-wire.steam-wire/machine_token.json` on macOS
    /// - `%LocalAppData%/steam-wire/steam-wire/cache/machine_token.json` on Windows
    pub fn user_cache() -> Self {
        let project_dirs = ProjectDirs::from("", "steam-wire", "steam-wire")
    ...
    fn all_tokens(&self) -> Result<HashMap<String, String>, FileStoreError> {
        ...
        let raw = read_to_string(&self.path)
            .map_err(|err| FileStoreError::Read { err, path: self.path.clone() })?;
        serde_json::from_str(&raw)
            .map_err(|err| FileStoreError::Json { err, path: self.path.clone() })
    }

    fn save(&self, tokens: HashMap<String, String>) -> Result<(), FileStoreError> {
        if let Some(parent) = self.path.parent() {
            create_dir_all(parent)
                .map_err(|err| FileStoreError::DirCreation { err, path: parent.into() })?;
        }

        let raw = serde_json::to_string(&tokens)
            .map_err(|err| FileStoreError::Json { err, path: self.path.clone() })?;
        write(&self.path, raw)
            .map_err(|err| FileStoreError::Write { err, path: self.path.clone() })?;
        Ok(())
    }
}
