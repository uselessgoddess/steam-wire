...
use prost::Message;
use steam_wire_crypto::CryptError;
use steam_wire_proto_common::{MsgKind, MsgKindEnum};
use steam_wire_proto_steam::CMsgProtoBufHeader;
...
impl NetMessageHeader {
    fn read<R: ReadBytesExt + Seek>(
            ...
            || kind == EMsg::KEMsgChannelEncryptResult
        {
            ...
            let steam_id = reader.read_u64::<LittleEndian>()?.try_into().unwrap_or(steam_id_nil());
...
impl RawNetMessage {
    pub fn read<Body: Into<Bytes>>(body: Body) -> Result<Self> {
        ...
        let kind = reader.read_u32::<LittleEndian>().map_err(|_| NetworkError::InvalidHeader)?;
...
impl RawNetMessage {
    pub fn into_header_and_message<T: NetMessage>(self) -> Result<(NetMessageHeader, T)> {
        ...
        if self.kind == T::KIND {
            trace!("reading body of {:?} message({} bytes)", self.kind, self.data.len());
