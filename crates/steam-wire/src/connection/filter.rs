...
use steam_wire_proto_common::MsgKind;
use steam_wire_proto_steam::EMsg;
use tokio::spawn;
...
impl MessageFilter {
            ...
            while let Some(res) = source.next().await {
                match res {
                    Ok(message) => {
                        ...
                        if let Some((_, tx)) =
                            filter_send.job_id_filters.remove(&message.header.target_job_id)
                        {
                            tx.send(message).ok();
                        } else if let Some(map_ref) =
                            filter_send.job_id_multi_filters.get(&message.header.target_job_id)
                        {
                    ...
                    Err(err) => {
                        ...
                        if let NetworkError::Ws(_) | NetworkError::IO(_) | NetworkError::EOF = err {
                            debug!(error = ?err, "transport disconnected, stopping filter task");
                            break;
                        } else {
                            error!(error = ?err, "failed to decode incoming steam message");
                        }
                    }
                }
            }
    ...
    pub fn on_notification(
        ...
        let tx =
            self.notification_filters.entry(job_name).or_insert_with(|| broadcast::channel(16).0);
        tx.subscribe()
    }

    pub fn on_kind<K: Into<MsgKind>>(&self, kind: K) -> broadcast::Receiver<RawNetMessage> {
        let tx = self.kind_filters.entry(kind.into()).or_insert_with(|| broadcast::channel(16).0);
        tx.subscribe()
    }
