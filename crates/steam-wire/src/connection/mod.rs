...
use steam_wire_proto_common::{GCHandshake, JobMultiple, MsgKindEnum};
...
impl Connection {
    ...
    pub async fn anonymous(server_list: &ServerList) -> Result<Self, ConnectionError> {
        UnAuthenticatedConnection::connect(server_list).await?.anonymous().await
    }
    ...
    pub async fn anonymous_server(server_list: &ServerList) -> Result<Self, ConnectionError> {
        UnAuthenticatedConnection::connect(server_list).await?.anonymous_server().await
    }
    ...
    pub async fn access(
        ...
        UnAuthenticatedConnection::connect(server_list).await?.access(account, access_token).await
    }
    ...
    pub fn web_access_token(&self) -> Option<&str> {
        self.session().web_access_token.as_deref()
    }
...
pub trait ReadonlyConnection {
    ...
    fn one_with_header<T: NetMessage + 'static>(
        &self,
    ) -> impl Future<Output = Result<(NetMessageHeader, T)>> + Send + 'static;

    /// Wait for one message of a specific kind
    fn one<T: NetMessage + 'static>(&self) -> impl Future<Output = Result<T>> + Send + 'static;

    /// Listen to messages of a specific kind, also returning the header
    fn on_with_header<T: NetMessage + 'static>(
        &self,
    ) -> impl Stream<Item = Result<(NetMessageHeader, T)>> + Send + 'static;

    /// Listen to messages of a specific kind
    fn on<T: NetMessage + 'static>(&self) -> impl Stream<Item = Result<T>> + Send + 'static;
}
