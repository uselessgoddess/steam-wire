...
impl Default for ConsoleAuthConfirmationHandler {
    fn default() -> Self {
        ConsoleAuthConfirmationHandler { input: BufReader::new(stdin()), output: stdout() }
    }
}
    ...
    Write: AsyncWrite + Unpin + Send + Sync,
{
    ...
    pub fn new(input: Read, output: Write) -> Self {
        UserProvidedAuthConfirmationHandler { input: BufReader::new(input), output }
    }
}
    ...
    async fn handle_confirmation(
        ...
        for method in allowed_confirmations {
            if let Some(token_type) = method.token_type() {
                let msg =
                    format!("{}: {}", method.confirmation_type(), method.confirmation_details());
...
impl SharedSecretAuthConfirmationHandler {
    ...
    pub fn new(shared_secret: &str) -> Self {
        SharedSecretAuthConfirmationHandler { shared_secret: shared_secret.into() }
    }
}
...
impl AuthConfirmationHandler for SharedSecretAuthConfirmationHandler {
    async fn handle_confirmation(
        ...
        for method in allowed_confirmations {
            if let Some(token_type) = method.token_type() {
                let auth_code = totp::auth_code(self.shared_secret, None)
